...
https://tra38.github.io/Paranoia_Super_Mission_Generator/

The online version is a HTML/CSS/JS conversion of the original Ruby code, generated by OpenAI Codex. It has some additional features not available in the Ruby version, such as:

- Can edit the generated mission within the application
- Allow export as HTML and Markdown (while also retaining itty.bitty.site)
- Printing
- Ability to regenerate specific content that uses Generative AI. (The vast majority of content in the application is human-generated, but if AI-generated content gets used, you can press the button "Regenerate the Entry" to replace that content with human-generated content.)
...
- If you want to run the HTML conversion, go to ```docs/```.

- If you want to run ```main.rb```, type ```ruby main.rb```. If you want to run ```railroad.rb```, type ```ruby railroad.rb```. And, if you want to run ```custom_names.rb```, type ```ruby custom_names.rb```.

The rest of the README assumes you are looking at the original Ruby code.
